...
uint32_t decodeULEB128(void*& ptr)
{
	...
	for (int i = 0;; i++) {
		assert(i < 5,
			"Error: attempted to decode ULEB128 integer larger than uint32_t");
...
int decodeSLEB128(void*& ptr)
{
	...
	while (true) {
		assert(shift < 32,
			"Error: attempted to decode SLEB128 integer larger than int");
		byte = *(char*)ptr;
		ptr = (void*)((uint32_t)ptr + 1);
		out |= (byte & 0x7f) << shift;
		shift += 7;
...
DWARF::DWARF(ELF* e)
{
	...
	while ((uint32_t)ptr < end) {
		...
		assert(header.addr_size == 4,
			"Error: don't know how to handle differently sized addresses");
...
Tuple<uint32_t, String> DWARF::getLineForAddr(void* ptr)
{
	...
	// assert(header -> version == 4, "Error: don't know how to parse CU's with
	// version != 4");
	assert(header->addr_size == 4,
		"Error: don't know how to handle differently sized addresses");
	...
	assert(abbrevs->contains(name) && (abbrevs->get(name)->tag_type == DW_TAG_compile_unit),
		"Error: somehow CU doesn't start with compilation unit");
...
DWARFLineStateMachine::DWARFLineStateMachine(uint32_t index, ELF* e)
{
	...
	assert(
		...
		"Error: don't know how to deal with nonstandard opcodes"); // FIXME this
																   // is kind of a
																   // hack
...
// TODO why are we making so many Maybe<Interval>s? This happens elsewhere in
// this file.
Maybe<Tuple<uint32_t, String>>
DWARFLineStateMachine::getLineForAddr(void* ptr)
{
	reset();
