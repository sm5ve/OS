...
ELF::ELF(void* b)
{
	...
	assert((header->magic[0] == 0x7f) && (header->magic[1] == 'E') && (header->magic[2] == 'L') && (header->magic[3] == 'F'),
		"Error: invalid ELF header");
	assert(header->elf_class == ELF_CLASS_32,
		"Error: we do not support 64 bit binaries yet!");
	assert(header->osabi == ELF_OSABI_SYSTEM_V, "Error: erroneous OS ABI");
	assert(header->architecture == ELF_ARCH_386,
		"Error: we're on a 386 ya dingus!");
	assert(getSectionHeader(header->section_header_string_tbl_index) == getSectionHeader(".shstrtab"),
		"Error: strtab mismatch");
}

ELF::~ELF() {}

ELFHeader32* ELF::getHeader32() { return (ELFHeader32*)base; }

ELFProgramHeader32* ELF::getProgramHeader(uint32_t index)
{
	assert(index < getHeader32()->prog_header_entry_count,
		"Error: tried to get out of bounds segment");
...
{
	assert(index < getHeader32()->section_header_entry_count,
		"Error: tried to index out of bounds section");
...
PrintStream& operator<<(PrintStream& p, const ELFHeader32* h)
{
	...
	case ELF_DATA_NONE:
		...
		break;
	case ELF_DATA2_LSB:
		p << "2's compliment little endian\n";
		break;
	case ELF_DATA2_MSB:
		p << "2's compliment big endian\n";
		break;
	default:
		p << (int)(h->data) << " (unknown)\n";
		break;
	}
	p << "OS/ABI:\t\t\t\t";
	switch (h->osabi) {
	case ELF_OSABI_SYSTEM_V:
		p << "System V\n";
		break;
	case ELF_OSABI_HPUX:
		p << "HP UX\n";
	...
	p << "Start of section headers:\t" << (h->section_header_off)
	  << " bytes from start\n";
	p << "Section header entry size:\t" << (h->section_header_entry_size)
	  << " bytes\n";
