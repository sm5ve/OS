...
void init()
{
	...
	for (uint32_t i = 0; i < entry_count; i++) {
		size_t region_size = (entries[i].ending_bus - entries[i].starting_bus + 1)
			<< 20;
		...
		for (uint32_t bus = entries[i].starting_bus; bus < entries[i].ending_bus;
			...
			for (uint32_t device = 0; device < 32; device++) {
				...
				if (*vendor != 0xffff) {
					present_devices.push((phys_addr)(
						entries[i].base_address + ((bus - entries[i].starting_bus) << 20) + (device << 15)));
				}
			}
		}
