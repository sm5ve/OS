...
InterruptHandlerDecision handleIRQ(registers&, void*)
{
	if (wait_ticks == 0) {
		waiting = false;
	} else {
...
{
	outb(PIT_MODE, 0x30);
	IDT::installIRQHandler(handleIRQ, 0, NULL);
}
...
// Note: this does NOT support multiple timers, and may very well be a temporary
// solution while we get SMP up and running
...
void waitMillis(uint32_t ms) { waitMicros(ms * 1000); }
} // namespace PIT
