...
virt_addr BootstrapPaging::mapRangeAfter(Interval<phys_addr> range,
	virt_addr after)
{
...
void BootstrapPaging::mapRange(Interval<phys_addr> physicalRange,
	virt_addr base)
{
	...
	for (uint32_t i = start; i < end; i += PAGE_SIZE) {
		addMapping((phys_addr)i, (virt_addr)(base + (i - (uint32_t)start)),
			PAGE_ENABLE_WRITE | PAGE_PRESENT);
	}
}
...
void BootstrapPaging::addMapping(phys_addr p, virt_addr v, uint32_t flags)
{
	...
	if (!isPresent(directory[directoryIndex])) {
		void* pageTable = kalloc_permanent(
			4096,
			4096); // FIXME This is a TERRIBLE way of allocating memory for page
				   // tables. We should just allocate a page and stash it in there.
...
Vector<shared_ptr<MemoryManager::PhysicalMemoryRegion>> BootstrapPaging::getRegions()
{
	Vector<shared_ptr<MemoryManager::PhysicalMemoryRegion>> out;
	Vector<page_table*>* current_region = new Vector<page_table*>();
	for (int i = 0; i < 1024; i++) {
		if (!isPresent(directory[i])) {
			if (current_region->size() > 0) {
				out.push(make_shared<MemoryManager::PhysicalMemoryRegion>(
					*current_region, (current_region->size()) * (1024 * PAGE_SIZE)));
				delete current_region;
				current_region = new Vector<page_table*>();
			}
		} else {
			page_table* tbl = (page_table*)physicalToPageTableAddr(
				(phys_addr)(directory[i] & (~0xfff)));
