...
Tuple<virt_addr, phys_addr>
reserveRangeOfSize(size_t size, IntervalSet<uint32_t>* memory_regions,
	BootstrapPaging* bspd)
{
	...
	return Tuple<virt_addr, phys_addr>(
		bspd->mapRangeAfter(
			Interval<phys_addr>((phys_addr)start, (phys_addr)end - 1),
			(virt_addr)0xc0000000),
		(phys_addr)start);
}

PageFrameAllocator* makeAllocatorForRange(Interval<uint32_t>& range,
	BootstrapPaging& pd)
{
	...
	// These size calculations are inaccurate and over-allocate. I am fine with
	// this.
	...
	range = Interval<uint32_t>(aligned_start + buffer_size + free_buff_size,
		aligned_end - 1);

	auto ptr_buff_interval = Interval<phys_addr>(
		(phys_addr)aligned_start, (phys_addr)(aligned_start + buffer_size - 1));
	auto free_buff_interval = Interval<phys_addr>(
		(phys_addr)(aligned_start + buffer_size),
		(phys_addr)(aligned_start + buffer_size + free_buff_size - 1));
	...
	return new PageFrameAllocator(
		size - (buffer_size + free_buff_size), (phys_addr*)ptr_buff,
		(uint32_t*)free_buff,
		(phys_addr)(aligned_start + buffer_size + free_buff_size));
}
...
void init(mboot_mmap_entry* entries, uint32_t len)
{
	...
	bspd->mapRange(Interval<phys_addr>((phys_addr)0, (phys_addr)0x01000000 - 1),
		(virt_addr)0xc0000000);
	bspd->install();
	...
	while ((uint32_t)ent < (uint32_t)entries + len) {
		if (ent->type == MEM_AVAILABLE) {
			memory_regions->add(
				Interval<uint32_t>(ent->addr, ent->addr + ent->len - 1));
		}
		ent = (mboot_mmap_entry*)((uint32_t)ent + (ent->size) + sizeof(void*));
	}

	memory_regions->subtract(Interval<uint32_t>(0x00000000, 0x01000000 - 1));
	// Perhaps we want to be a bit smarter about this in the future? 8MB of page
	// tables Is certainly a safe size to allocate for 32 bit systems, but we
	// might not always Need it
	auto table_store_mem = reserveRangeOfSize(8 * MB, memory_regions, bspd);
	auto free_ptbls_map_mem = reserveRangeOfSize(
		8 * MB / (sizeof(page_table) * 8), memory_regions, bspd);
...
void freePageTable(page_table* ptbl) { pageTableAllocator->free(ptbl); }
...
PageFrameAllocator::PageFrameAllocator(size_t size, phys_addr* ptr_buffer,
	uint32_t* free_buffer, phys_addr start)
{
	assert(size % PAGE_SIZE == 0,
		"Error: size of range not multiple of page size");
	assert((uint32_t)start % PAGE_SIZE == 0,
		"Error: misaligned start of allocation range");
...
{
	assert(((uint32_t)addr >= (uint32_t)base) && ((uint32_t)addr < (uint32_t)base + sz),
		"Error: tried to free page out of range from allocator");
	assert((uint32_t)addr % PAGE_SIZE == 0,
		"Error: tried to free misaligned page");
	free_index--;
...
size_t PageFrameAllocator::grow(PhysicalMemoryRegion& region,
	size_t target_size)
{
