...
void initKalloc()
{
	...
	new (&slab_allocs[0])
		SlabAlloc(SLAB_8_BUFF, KERNEL_SLAB_8_SIZE, 8, SLAB_8_FREED_MAP);
	new (&slab_allocs[1])
		SlabAlloc(SLAB_16_BUFF, KERNEL_SLAB_16_SIZE, 16, SLAB_16_FREED_MAP);
	new (&slab_allocs[2])
		SlabAlloc(SLAB_32_BUFF, KERNEL_SLAB_32_SIZE, 32, SLAB_32_FREED_MAP);
	new (&slab_allocs[3])
		SlabAlloc(SLAB_64_BUFF, KERNEL_SLAB_64_SIZE, 64, SLAB_64_FREED_MAP);

	heap_alloc = (HeapAlloc*)uninitialized_heap_alloc;
	new (heap_alloc) HeapAlloc(KERNEL_HEAP, KERNEL_HEAP_SIZE, KERNEL_HEAP_PTRS,
		KERNEL_HEAP_GRANULARITY);
	initBumpHeap();
}
