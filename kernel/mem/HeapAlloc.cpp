...
HeapAlloc::~HeapAlloc() { assert(false, "I don't know what to do here"); }
...
BinaryTreeNode<size_t>*
HeapAlloc::findNextBiggestNode(size_t size, BinaryTreeNode<size_t>* node)
{
	if (node == NULL) {
		node = heap->getRoot();
	}
...
void* HeapAlloc::shrinkNode(BinaryTreeNode<size_t>* node, size_t by_amount)
{
	...
	assert(node->getValue() >= by_amount,
		"Error: tried to shrink free block for more than its total size");
...
void* HeapAlloc::alloc(size_t size)
{
	...
	assert(((uint32_t)to_shrink - (uint32_t)buffer) % granularity == 0,
		"Error: allocated misaligned memory\n");
...
void HeapAlloc::mergeAdjacentChunks(BinaryTreeNode<size_t>* left,
	BinaryTreeNode<size_t>* right)
{
	assert(left < right, "Error: mixed up arguments for mergeAdjacentChunks");
	assert((uint32_t)left + left->getValue() == (uint32_t)right,
		"Error: chunks not adjacent");
...
void HeapAlloc::free(void* ptr)
{
	...
	assert(((uint32_t)ptr - (uint32_t)buffer) % granularity != 4,
		"Error: I think you deleted where you should've delete[]'d");
	assert(((uint32_t)ptr - (uint32_t)buffer) % granularity == 0,
		"Error: tried to free misaligned pointer");
