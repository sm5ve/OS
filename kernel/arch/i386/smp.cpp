...
void startCore(uint8_t apic_id, uint16_t offset)
{
	...
	PIT::waitMicros(200);
	if (online_cores == prev_online) { // If our core count doesn't go up, try
...
void init()
{
	...
	apic[0xf0 / 4] |= (1 << 8);
	memcpy((void*)(0xc0000000 + ap_boot_addr), &bootup_start,
		((uint32_t)&bootup_end - (uint32_t)&bootup_start));
