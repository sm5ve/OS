...
// But that might be best to implement after we move over to a higher-half
// kernel And solidify the memory map
...
void writeSegment(uint8_t* table, int index, uint32_t base, uint32_t limit,
	uint8_t access)
{
...
void flushGDT()
{
	gdtDescriptor = { .size = 8 * TOTAL_GDT_SEGMENTS - 1, .addr = (uint32_t)gdt };
	// Actually let the CPU know where the GDT is
	__asm__ volatile("lgdtl %0" ::"m"(gdtDescriptor)
					 : "memory");
	...
	__asm__ volatile("mov $0x2b, %ax \n"
					 "ltr %ax");
	// Do we maybe want to separate the segment register flushes to another
	// function? Maybe it's not necessary for each GDT flush?
}

constexpr uint8_t segmentFlags(bool kernel, bool system, bool executable,
	bool rw)
{
	return (uint8_t)(
		...
		((system ? 0 : 1)
			<< 4)
		|										 // If bit 4 is unset, we declare this a system segment
		((executable ? 1 : 0) << 3) | (0 << 2) | // We'll always assume our segment grows up
		((rw ? 1 : 0)
			<< 1)
		|		   // If this is a code segment, rw = false prevents reading from
				   // the segment, if this is a data segment, rw = false prevents
				   // writing to this segment
		(0 << 0)); // CPU sets this flag if the segment is ever accessed
}

void installGDT()
{
	writeSegment(gdt, 0, 0x00000000, 0x00000000,
		0); // GDT must start with null segment
...
void writeAPBootstrapGDT(uint16_t offset)
{
	...
	seg_table_descriptor desc { .size = 8 * 3,
